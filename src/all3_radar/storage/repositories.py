...
class RadarRepository:
    ...
    def create_digest_vote_round(
        ...
        *,
        ...
        vote_round_id = uuid.uuid4().hex
        now = _utc_now_iso()
        with connect(self.database_path) as connection:
            connection.execute(
                """
                INSERT INTO digest_vote_rounds (
                  id, pipeline_run_id, week_key, status, seats_to_fill, shortlisted_count, candidate_count,
                  summary_json, telegram_chat_id, telegram_message_id, created_at, updated_at, closed_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?, NULL)
                """,
                (
            ...
            connection.commit()
        return vote_round_id

    def replace_digest_vote_candidates(
        self,
        vote_round_id: str,
        *,
        ...
        for index, candidate in enumerate(shortlisted_candidates, start=1):
            rows.append(
                (
                    vote_round_id,
                    str(candidate["canonical_event_id"]),
                    str(candidate["normalized_item_id"]),
                    index,
                    int(candidate["score"]),
                    1,
                    json.dumps(
        ...
        offset = len(shortlisted_candidates)
        ...
        with connect(self.database_path) as connection:
            connection.execute("DELETE FROM digest_vote_candidates WHERE vote_round_id = ?", (vote_round_id,))
            connection.executemany(
                """
                INSERT INTO digest_vote_candidates (
                  vote_round_id, canonical_event_id, normalized_item_id, candidate_rank, score, is_preselected, rationale_json
                )
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                rows,
    ...
    def attach_digest_vote_round_message(
        self,
        vote_round_id: str,
        *,
        ...
        with connect(self.database_path) as connection:
            connection.execute(
                """
                UPDATE digest_vote_rounds
                SET telegram_chat_id = ?, telegram_message_id = ?, updated_at = ?
                WHERE id = ?
                """,
                (telegram_chat_id, telegram_message_id, _utc_now_iso(), vote_round_id),
            )
            connection.commit()

    def get_digest_vote_round(self, vote_round_id: str) -> dict[str, Any] | None:
        with connect(self.database_path) as connection:
            row = connection.execute(
                """
                SELECT id, week_key, status, seats_to_fill, shortlisted_count, candidate_count,
    ...
    def has_digest_vote_candidate(self, vote_round_id: str, canonical_event_id: str) -> bool:
        with connect(self.database_path) as connection:
            row = connection.execute(
                """
                SELECT 1
                FROM digest_vote_candidates
                WHERE vote_round_id = ? AND canonical_event_id = ?
                LIMIT 1
                """,
                (vote_round_id, canonical_event_id),
            ).fetchone()
        return row is not None

    def get_digest_vote_state(
        self,
        *,
        vote_round_id: str,
        ...
        with connect(self.database_path) as connection:
            row = connection.execute(
                ...
                LIMIT 1
                """,
                (vote_round_id, canonical_event_id, voter_user_id, actor_chat_id),
            ).fetchone()
        return bool(row["is_active"]) if row else False

    def count_active_digest_votes_for_voter(
        self,
        *,
        vote_round_id: str,
        voter_user_id: str,
        ...
        with connect(self.database_path) as connection:
            row = connection.execute(
                ...
                WHERE vote_round_id = ?
                  ...
                  AND is_active = 1
                """,
                (vote_round_id, voter_user_id, actor_chat_id),
            ).fetchone()
        return int(row[0]) if row else 0

    def upsert_digest_vote(
        self,
        *,
        vote_round_id: str,
        canonical_event_id: str,
        ...
        with connect(self.database_path) as connection:
            connection.execute(
                """
                INSERT INTO digest_votes (
                  id, vote_round_id, canonical_event_id, voter_user_id, actor_chat_id, is_active, created_at, updated_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(vote_round_id, canonical_event_id, voter_user_id, actor_chat_id) DO UPDATE SET
    ...
    def load_digest_vote_candidates_with_totals(self, vote_round_id: str) -> list[dict[str, Any]]:
        with connect(self.database_path) as connection:
            rows = connection.execute(
                """
                SELECT dvc.vote_round_id,
                       dvc.canonical_event_id,
                       dvc.normalized_item_id,
                       dvc.candidate_rank,
                       dvc.score,
                       dvc.is_preselected,
                       dvc.rationale_json,
                       COUNT(CASE WHEN dv.is_active = 1 THEN 1 END) AS active_votes
                FROM digest_vote_candidates dvc
                LEFT JOIN digest_votes dv
                  ON dv.vote_round_id = dvc.vote_round_id
                 AND dv.canonical_event_id = dvc.canonical_event_id
                WHERE dvc.vote_round_id = ?
                GROUP BY
                  dvc.vote_round_id,
                  dvc.canonical_event_id,
                  dvc.normalized_item_id,
                  ...
                  dvc.is_preselected,
                  dvc.rationale_json
                ORDER BY dvc.candidate_rank ASC, dvc.score DESC, dvc.canonical_event_id ASC
                """,
                (vote_round_id,),
            ).fetchall()
        return [dict(row) for row in rows]

    def close_digest_vote_round(self, vote_round_id: str) -> None:
        with connect(self.database_path) as connection:
            connection.execute(
                """
                UPDATE digest_vote_rounds
                ...
                WHERE id = ?
                """,
                ("closed", _utc_now_iso(), _utc_now_iso(), vote_round_id),
            )
            connection.commit()
