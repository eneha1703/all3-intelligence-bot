"""Parser for Crunchbase News archive pages and article pages."""
...
ARTICLE_PATH_RE = re.compile(r"^/(?!sections/|category/|tag/|author/|wp-json/)[^/]+/[^/?#]+/?$")
...
class _CrunchbaseNewsListingParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if not href:
            return
        if "h2" not in self._tag_stack:
            return
        absolute = urljoin(self.base_url, href)
        path = urlparse(absolute).path
        if ARTICLE_PATH_RE.match(path) and absolute not in self.article_urls:
...
def _extract_article_urls(listing_html: str, base_url: str) -> list[str]:
    parser = _CrunchbaseNewsListingParser(base_url=base_url)
    parser.feed(listing_html)
    parser.close()
    return parser.article_urls


def parse_crunchbase_news_article(article_html: str) -> ParsedCrunchbaseArticle:
    ...
    if not title:
        raise ValueError("Crunchbase News parser could not extract a title")

    published_ts = None
    for key in ("article:published_time", "og:published_time", "publish-date", "date", "dc.date"):
    ...
    return ParsedCrunchbaseArticle(title=title, published_ts=published_ts, snippet=snippet)


def parse_crunchbase_news_listing(
    listing_html: str,
    ...
    for article_url in article_urls[:article_limit]:
        ...
        items.append(
            CollectedRawItem(
                ...
                raw_payload={"url": article_url, "title": parsed.title, "source_url": source.url},
            )
        )

    if not items:
        if skipped_missing_dates:
            raise ValueError("Crunchbase News parser found article links but could not extract trustworthy published dates")
        raise ValueError("Crunchbase News parser did not extract any usable article items")
